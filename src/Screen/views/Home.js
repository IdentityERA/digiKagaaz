...
export default function Home(){
    ...
    const headers= { 'Content-Type': 'application/json',
                     'Authorization': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MjhjYmYwZDE3MjY4ZDQwZjFiMGYyNzMiLCJpYXQiOjE2NTMzOTUzNjQsImV4cCI6MTY1NDI1OTM2NH0.9ddtkOqK08TojwuIqXJH2HOw-KrwJhZndF2s5w7HYBk'
                    }
    React.useEffect(() =>{
        ...
        async function fetchAPI(){
            try{
                ...
                const response = await axios.get("http://localhost:3001/roles/",{headers:headers});
                const allScopes = await axios.get("http://localhost:3001/scopes/", {headers:headers});
    ...
    const getSelectedVal = async (value) => {
        ...
        try{
            ...
            const rolescopes = await axios.get("http://localhost:3001/roles/"+roleId.data+"/scope",{headers:headers})
    ...
    function getPermissionRows(permissions,b){
        ...
        const permisions =  listname.filter((listname, function(val){return val.indexOf( b ) !== -1;}));
        return permisions.map(function(obj, i){
            return(
                <td className = "centertext" key= {i}><input id={obj} key= {i} type="checkbox" value ={obj} checked={checkForExisitance(obj,onAPI)} onChange={()=>changePermissionState(obj)} className ="switch"/></td>
            )
        })
    }
            ...
            <div>
                <div className="container">
                    <div className='roles-scope-box'>
                        <h2 className='rc-header'><strong>Roles and Scope</strong></h2>
                        ...
                        <div className="container table-responsive"> 
                            ...
                            <thead className="thead-dark">
                                <tr>
                                <th className = "apiendpoint" scope="col">Operations</th>
                                <th className = "centertext" scope="col">Create</th>
                                <th className = "centertext" scope="col">Read</th>
                                <th className = "centertext" scope="col">Update</th>
                                <th className = "centertext" scope="col">Delete</th>
                                </tr>
                            </thead>
                            <tbody>
                                {getApiRows()}
